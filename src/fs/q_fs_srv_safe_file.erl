...
%%%
%%% @end
%%% Created : 08. Jul 2017 9:09 AM
%%%-------------------------------------------------------------------
-module(q_fs_srv_safe_file).
-author("madalin").
...
init([]) ->
    ...
    State = #q_fs_srv_safe_file_state{},
...
%%=============================================
%% handle_call {write, Filename, Bytes}
%%=============================================

%% Writes the contents of the iodata term Bytes to file Filename
handle_call({write, Filename, Bytes}, _From, State) ->

    {reply, utils_write(Filename, Bytes), State};

%%=============================================
%% handle_call {read, Filename}
%%=============================================

%% Sets universe option
handle_call({read, Filename}, _From, State) ->

    {reply, utils_read(Filename), State};
...
%% @doc Write file
utils_write(Filename, Bytes) ->

    %% Build backup file name and backup checksum file name
    BackupFilename          = filename:append(Filename, ".bak"),
    BackupChecksumFilename  = filename:append(BackupFilename, ".crc"),

    %% Restore any existing backup file
    utils_restore_backup_if_valid(Filename, BackupFilename, BackupChecksumFilename),

    %% Create another backup file in case we fail the write
    action_create_backup(Filename),

    %% Write the file
    action_write(Filename, Bytes),

    %% Delete the backup so the backup will not be restored next time we attempt to write
    action_delete_backup(Filename),

    ok.

%% @doc Read file
utils_read(Filename, Bytes) ->

    %% Build backup file name and backup checksum file name
    BackupFilename          = filename:append(Filename, ".bak"),
    BackupChecksumFilename  = filename:append(BackupFilename, ".crc"),

    %% Restore any existing valid backup file
    utils_restore_backup_if_valid(Filename, BackupFilename, BackupChecksumFilename),
...
%% @doc Delete the specified backup file
utils_delete_backup(BackupFilename, BackupChecksumFilename) ->

    case file:delete(BackupChecksumFilename) of
        ok ->
            case file:delete(BackupFilename) of
                ok -> ok;
                Error2 -> ?Q_ERROR(Error2)
            end;
        Error -> ?Q_ERROR(Error)
    end.

%% @doc Restore backup if any
utils_restore_backup_if_valid(Filename, BackupFilename, BackupChecksumFilename) ->

    %% Check if backup file exist
    case filelib:is_file(BackupFilename) of
        true ->

            %% Check if backup crc32 exists
            case filelib:is_file(BackupChecksumFilename) of
                true ->

                    %% Restore backup
                    utils_restore_backup(Filename, BackupFilename, BackupChecksumFilename);
        ...
        false ->
            ...
            %% Do nothing - no backup was found
            ok
    end.

%% @doc Restore the specified backup file
utils_restore_backup(Filename, BackupFilename, BackupChecksumFilename) ->

    %% Read backup file crc32
    case file:read_file(BackupChecksumFilename) of
        {ok, <<SizeOfChecksum:8/big-unsigned-integer, Checksum:SizeOfChecksum/big-unsigned-integer>>} ->

            %% Check backup file CRC32
            case q_utils_crc32:check_file(BackupFilename, Checksum) of
                true ->

                     %% Restore the backup file
                     case file:copy(BackupFilename, Filename) of
                         {ok, _BytesCopied} -> ok;
                         CopyError ->
                ...
                false ->
                    ...
                    %% Invalid checksum
                    ?Q_ERROR(fs_safe_file_invalid_backup_ckecksum);

                Crc32CheckFileError ->

                    %% Crc32 check file error
                    Crc32CheckFileError

            end;

        {ok, _} -> ckecksum_invalid_file_format;
        ReadFileError ->

            ?Q_LOG(error, checksum_read_file_error, ReadFileError),
            checksum_read_file_error
